...
export const updatePersonalSubtaskById = (req, res) => {
    ...
    const query = `
        ...
        SET 
            ...
            ps.status = ?
            ps.employee_id
        WHERE ps.personalsubtask_id = ? AND u.user_id = ?`;
...
export const getAllPersonalSubtask = (req, res) => {
    ...
    const personalSubtaskQuery = `
        ...
        INNER JOIN employee e ON ps.employee_id = e.employee_id
        INNER JOIN users u ON e.user_account_id = u.user_id
        WHERE ps.project_id = ? AND u.user_id = ?
    `;
    ...
    Promise.all([
        new Promise((resolve, reject) => {
            db.query(personalSubtaskQuery, [projectId, userId], (err, personalSubtasks) => {
                if (err) reject(err);
...
export const getProjectPriorityBasedPersonalSubtask = (req, res) => {
    ...
    const query = `
        ...
        INNER JOIN employee e ON ps.employee_id = e.employee_id
        INNER JOIN users u ON e.user_account_id = u.user_id
        WHERE ps.project_id = ? AND ps.Priority = ? AND ps.is_deleted = 0 AND u.user_id = ?
    `;
    const values = [projectId, priority, req.userId];
...
export const getPriorityPersonalSubtaskCount = (req, res) => {
    const { userId } = req.params; // Assuming userId is available from checkAuth middleware
    const query = `
        SELECT
